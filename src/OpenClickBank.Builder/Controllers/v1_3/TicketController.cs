...
public class TicketController : ControllerBase
{
    ...
    [ApiAuthorizationFilter(new[] { ApiPermission.ApiOrderWrite, ApiPermission.HasDeveloperKey })]
    ...
    public ActionResult CreateTicket(
        ...
        [Required, FromQuery, SwaggerParameter(Description = "The type of the ticket. Must be either 'rfnd', 'cncl' or 'tech'. For 'rfnd' the parameter refundType must also be specified. If the receipt is for a non-recurring product, either 'rfnd' or 'cncl' will automatically refund that sale. For any receipt of a recurring product, a 'rfnd' will refund that receipt AND cancel any future billing, while a 'cncl' will only cancel future billing without issuing any refunds.")] QueryTicketType? type,
...
ticket.type.tech_support.10 (I never received my product.)")] string? reason,
    ...
    [Authorize]
    [Produces("application/json", "application/xml")]
    [ApiAuthorizationFilter(new[] { ApiPermission.ApiOrderRead })]
    ...
    public ActionResult GetTicketRefundAmounts(
        [Required, SwaggerParameter(Description = "A valid receipt id.")] string receipt,
        [SwaggerParameter(Description = "line item sku/itemNo.")] string? sku,
        ...
        [SwaggerParameter(Description = "Specified for partial refunds indicating the amount of the transaction to be refunded. For 'PARTIAL_PERCENT' this must be a number between 1 and 80, with no more than two digits of precision - for example 50.00. For 'PARTIAL_AMOUNT' this is the amount to refund in the currency the customer used during the purchase.")] decimal? refundAmount) => Ok();
    ...
    public ActionResult UpdateTicket(
        ...
        [FromQuery, SwaggerParameter(Description = "If changing the type of the ticket, this will be one of rfnd, cncl, or tech. Note: Partial refunds are not allowed when changing to a rfnd ticket type. Tickets changed to rfnd will be full refunds.")] QueryTicketType? type) => Ok();
